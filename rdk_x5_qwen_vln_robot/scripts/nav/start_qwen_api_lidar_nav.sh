...
echo "cmd_topic=/cmd_vel RUN_CHASSIS=$RUN_CHASSIS CHASSIS_PORT=${CHASSIS_PORT:-unset}"
...
echo "Started nav pid=$NAV_PID (lidar pid=${LIDAR_PID:-n/a})."
...
echo "Waiting for nav to exit; Ctrl+C or nav crash -> stop all (camera/lidar/chassis)."
