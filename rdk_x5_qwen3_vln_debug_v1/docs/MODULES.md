...
- `common.txt`：统一 JSON、`[0,1000]` 相对坐标协议和安全约束；
- `observe.txt`：判断目标是否可见，否则给出探索路点；
- `track.txt`：重新定位已锁定目标；
- `search.txt`：目标不可见时选择可见自由空间探索点；
...
- JPEG 编码和 Data URL；
- 严格 JSON、状态结果校验；
- 将模型 `[0,1000]` 相对坐标换算为 API 输入图像像素；
- 在解析失败时保留原始输出预览，便于定位问题。

模型输出协议是相对坐标；可视化与 ROS 话题使用换算后的像素坐标。
