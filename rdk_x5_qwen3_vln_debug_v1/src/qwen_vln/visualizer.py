...
class ResultVisualizer:
    def __init__(
        ...
        self.history: Deque[Tuple[PixelPoint, str]] = deque(maxlen=max(1, int(history_length)))
    ...
    def add_result(self, result: Optional[ModelResult]) -> None:
        if result is None or result.point is None or result.request_id == self._last_request_id:
            return
    ...
    def draw(
        ...
        if self.draw_center_line:
            cv2.line(canvas, (width // 2, 0), (width // 2, height), (90, 90, 90), 1)

        # Historical pixels only make geometric sense while the camera is fixed.
        # The default history length is 1, so V1 shows only the current result.
        ...
        for index, (point, role) in enumerate(historical):
            ...
            overlay = canvas.copy()
            cv2.circle(overlay, point.as_tuple(), max(3, self.point_radius // 3), color, -1)
        ...
        if result is not None and result.point is not None:
            ...
            cv2.line(canvas, (point[0] - r - 7, point[1]), (point[0] + r + 7, point[1]), color, 2)
            cv2.line(canvas, (point[0], point[1] - r - 7), (point[0], point[1] + r + 7), color, 2)
            self._label(
                canvas,
                f"{result.point_role.upper()} ({point[0]}, {point[1]})",
                (point[0] + 14, max(24, point[1] - 14)),
                color,
            )

        panel_height = 154 if error_text else 132
        overlay = canvas.copy()
        cv2.rectangle(overlay, (0, 0), (width, panel_height), (18, 18, 18), -1)
        canvas = cv2.addWeighted(overlay, 0.72, canvas, 0.28, 0)

        status = f"STATE: {state.value}" + ("  |  API REQUESTING" if request_in_flight else "")
        cv2.putText(canvas, status, (16, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.66, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(
            canvas,
            f"TASK: {self._shorten(instruction, 90)}",
            (16, 50),
        ...
        cv2.putText(canvas, f"REASON: {self._shorten(reason or '-', 90)}", (16, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.49, (205, 205, 205), 1, cv2.LINE_AA)
        cv2.putText(canvas, f"FRAME: {self._shorten(frame_note, 90)}", (16, 124), cv2.FONT_HERSHEY_SIMPLEX, 0.47, (180, 220, 255), 1, cv2.LINE_AA)
        if error_text:
            cv2.putText(canvas, f"ERROR: {self._shorten(error_text, 100)}", (16, 148), cv2.FONT_HERSHEY_SIMPLEX, 0.46, (80, 80, 255), 1, cv2.LINE_AA)
        return canvas
    ...
    def _label(image, text: str, origin: Tuple[int, int], color) -> None:
        ...
        cv2.rectangle(image, (x - 3, y - th - 5), (x + tw + 5, y + baseline + 3), (20, 20, 20), -1)
