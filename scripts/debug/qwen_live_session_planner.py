...
def load_pose_json(path: Path) -> Tuple[float, float, float]:
    ...
    if not isinstance(data, dict):
        raise ValueError(f"invalid pose json: {path}")
...
def resolve_robot_pixel(
    ...
    raise RuntimeError(
        f"机器人 map 坐标 ({x:.3f}, {y:.3f}) 附近 {search_radius}px 内找不到可通行白区。"
    )


def resolve_visited_mask(
    gray_nav: np.ndarray,
    meta: MapYamlMeta,
    map_yaml: Path,
    qwen_map_yaml: Optional[Path],
    traj_json: Optional[Path],
    *,
    corridor_radius_m: float = 0.35,
) -> np.ndarray:
    """从 qwen PGM 或 trajectory fallback 得到已扫区域 mask（候选筛选不使用）。"""
    h, w = gray_nav.shape
...
def build_nav_goal_payload(
    *,
    session_id: str,
    map_yaml: Path,
    meta,
    robot_map: Dict[str, float],
    chosen_pixel: Tuple[int, int],
    ...
    return {
        ...
        "safety": {
            ...
            "ready_for_nav2": True,
            "note": "Qwen 从程序验证候选中选出；Nav2 仍会做路径与碰撞检查。",
        },
    }


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Live session: v6 候选 + Qwen 选点（真实位姿）。")
    p.add_argument("--map-yaml", required=True, help="SLAM 保存的 map.yaml（Nav2 / 候选几何）")
    p.add_argument("--qwen-map-yaml", default="", help="Qwen 专用 map yaml（含已扫区域编码）；默认同目录 {stem}_qwen.yaml")
    p.add_argument("--pose-json", required=True, help="state/last_pose_map.json")
    p.add_argument("--trajectory-json", default="", help="trajectory_session.json（可选，画已走路径）")
    ...
    p.add_argument("--model-image-side", type=int, default=1600)
...
def build_v6_args(live_args: argparse.Namespace, pgm_path: Path, output_dir: Path):
...
def main() -> int:
    ...
    if gray is None:
        print(f"[FAIL] 无法读取地图：{pgm_path}", file=sys.stderr)
        return 2
    ...
    input_image, _ = v6.build_case_image(
        ...
        pose,
        case_candidates,
        v6_args.model_image_side,
        info_lines=[
            "LIVE SESSION INPUT",
            f"robot_map=({rx:.2f},{ry:.2f}) yaw={yaw_deg:.1f}",
            f"trajectory_vertices={len(vertices)}",
    ...
    if v6_args.dry_run:
        ...
        if not api_key:
            ...
            return 2
        try:
            raw_response, latency_s = v6.call_qwen(input_image, prompt, api_key, v6_args)
    ...
    report = {
        ...
        "robot_map_pose": {"x": rx, "y": ry, "yaw_deg": yaw_deg},
        "robot_pixel": {"x": robot_px, "y": robot_py},
        "selected_local_id": selected_local_id,
        "selected_global_id": chosen.global_id,
        "goal_pixel": {"x": chosen.x, "y": chosen.y},
