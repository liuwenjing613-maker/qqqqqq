#!/usr/bin/env bash
# 落地底盘测试入口
#   bash scripts/test_chassis_ground.sh direct          # 直连 M1，不经过 ROS（推荐先测）
#   bash scripts/test_chassis_ground.sh direct forward  # 只测前进
#   bash scripts/test_chassis_ground.sh ros             # 经 /cmd_vel 桥接测试（与 MVP 同路径）
#   bash scripts/test_chassis_ground.sh ros-bridge      # 仅启动底盘桥，供另一终端跑 ros 测试
#   bash scripts/test_chassis_ground.sh sweep           # 多档速度扫描（交互式）
...
stop_others() {
  ...
  bash "$PROJECT_DIR/scripts/stop_all_safe.sh" || true
  sleep 1
}
...
run_ros_bridge() {
  ...
  echo "     另开终端执行:"
  echo "     bash scripts/test_chassis_ground.sh ros $ACTION"
...
case "$MODE" in
  ...
  *)
    echo "用法:"
    echo "  bash scripts/test_chassis_ground.sh direct [forward|left|right|all]"
    echo "  bash scripts/test_chassis_ground.sh ros [forward|left|right|all]"
    echo "  bash scripts/test_chassis_ground.sh ros-bridge"
    echo "  bash scripts/test_chassis_ground.sh sweep"
    echo "  bash scripts/test_chassis_ground.sh rotate-sweep"
    exit 1
    ;;
esac
