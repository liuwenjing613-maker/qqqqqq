...
joy_topic_ready() {
  ros2 topic info /joy 2>/dev/null | grep -q "Type:"
}
...
wait_joy_topic() {
  ...
  for i in $(seq 1 "$max_wait"); do
    if joy_topic_ready; then
      return 0
    fi
    if ! pgrep -f "lib/joy/joy_node" >/dev/null 2>&1 && ! pgrep -f "joy/joy_node" >/dev/null 2>&1; then
      if [ "$i" -gt 5 ]; then
        echo "[joy] ERROR: joy_node exited early; see ${2:-logs/joy_teleop.log}"
        tail -n 8 "${2:-logs/joy_teleop.log}" 2>/dev/null || true
        return 1
      fi
    elif [ "$i" -ge 8 ]; then
      # joy_node is alive but DDS discovery is slow under heavy startup — don't block minutes.
      echo "[joy] WARN: /joy discovery slow (${i}x0.5s); joy_node running, continuing"
      return 0
    fi
    sleep 0.5
  done
...
start_joy_teleop() {
  ...
  local blocking="${JOY_TELEOP_BLOCKING:-1}"
  local joy_dev
  ...
  # SLAM stack may already publish /joy; reuse it instead of killing a working node.
  if joy_topic_ready && pgrep -f "joy/joy_node" >/dev/null 2>&1; then
    echo "[joy] reuse existing joy_node (/joy already live)"
  else
    pkill -f "teleop_twist_joy" 2>/dev/null || true
    pkill -f "joy/joy_node" 2>/dev/null || true
    pkill -f "lib/joy/joy_node" 2>/dev/null || true
    sleep 0.8
    ...
    if [ "$blocking" = "1" ]; then
      sleep 1.0
      if ! wait_joy_topic 24 "$log_file"; then
        echo "[joy] ERROR: /joy not available after joy_node start"
        return 1
      fi
    else
      echo "[joy] joy_node launched (non-blocking)"
      sleep 0.3
    fi
  fi

  # Always point teleop at /cmd_vel_joy (SLAM teleop may still be on /cmd_vel).
  pkill -f "teleop_twist_joy" 2>/dev/null || true
  sleep 0.2
  ...
  if [ "$blocking" = "1" ]; then
    sleep 1
    if ! pgrep -f "teleop_twist_joy" >/dev/null 2>&1; then
      echo "[joy] ERROR: teleop_node exited early; see ${log_file}"
