...
CONFIG="${1:-configs/nav_qwen_yolo.yaml}"
USER_INSTRUCTION="${2:-}"
NAV_ONLY="${NAV_ONLY:-0}"
...
echo "[V3] memory check:"
free -h || true
ollama ps || true
echo "[V3] run_shared_nav uses non-blocking mock Qwen verify in this phase."
...
if [ "$NAV_ONLY" = "1" ]; then
  echo "[V3] NAV_ONLY=1: starting shared nav only."
  python3 "$PROJECT_DIR/src/apps/run_shared_nav.py" \
    --config "$CONFIG" \
    --instruction "$INSTRUCTION" \
    > "$PROJECT_DIR/logs/qwen_yolo_nav.log" 2>&1 &
  echo "[V3] started nav pid=$!"
  exit 0
fi
...
if ros2 pkg prefix foxglove_bridge >/dev/null 2>&1; then
  ...
  bash "$PROJECT_DIR/scripts/lidar/start_foxglove.sh" \
    > "$PROJECT_DIR/logs/qwen_yolo_foxglove_bridge.log" 2>&1 &
fi

echo "[qwen_yolo_nav] started pid=$!"
echo "  tail -f logs/qwen_yolo_nav.log"
