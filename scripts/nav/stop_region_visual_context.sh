#!/usr/bin/env bash
# Stop only the region visual context node by verified PID.
set -Eeuo pipefail

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUNTIME_DIR="$PROJECT_DIR/runtime/qwen_visual_context"
META_FILE="$RUNTIME_DIR/process_meta.json"
PID_FILE="$RUNTIME_DIR/region_visual_context.pid"
...
if [[ ! -d "/proc/$PID" ]]; then
  echo "[STOP][PASS] visual context node already stopped (pid=$PID not running)"
...
rm -f "$PID_FILE" "$META_FILE"
echo "[STOP][PASS] target_pid=$PID target_identity_verified=true visual_context_node_stopped=true other_processes_targeted=0 broad_kill_used=false"
