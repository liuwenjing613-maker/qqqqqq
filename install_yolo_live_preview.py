#!/usr/bin/env python3
"""
Install a non-invasive YOLO-World live browser preview tool into this repository.

Usage:
  cd /path/to/qqqqqq
  python3 install_yolo_live_preview.py
...
LIVE_PREVIEW_PY = r'''#!/usr/bin/env python3
"""
...
def main():
    root = Path.cwd()
    expected = ["scripts", "debug_tools", "src"]
    missing = [name for name in expected if not (root / name).exists()]
    if missing:
        print("[WARN] 当前目录不像 qqqqqq 仓库根目录。缺少：", ", ".join(missing))
        print("       建议先 cd 到仓库根目录后再运行。")
        print("       继续写入当前目录。")

    write_file(root / "debug_tools" / "yolo_live_browser_preview.py", LIVE_PREVIEW_PY, executable=True)
    write_file(root / "scripts" / "start_yolo_live_preview.sh", START_SCRIPT, executable=True)
